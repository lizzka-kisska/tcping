...
class TestMain:
    ...
    def test_create_stat(self):
        ...
        assert create_stat() == 'Packets: 5 (50%) passed, 5 (50%) failed, 10 sent'
        const.passed, const.packages = 3, 11
        assert create_stat() == 'Packets: 3 (28%) passed, 8 (72%) failed, 11 sent'

    def test_create_time_res(self):
        const.time = [14, 16, 20, 6]
        assert create_time_stat() == 'Packet sending time: min - 6 ms, max - 20 ms, average - 14.0 ms'
        const.time = [14.543, 20.765, 15.577, 18.431]
        assert create_time_stat() == 'Packet sending time: min - 14.543 ms, max - 20.765 ms, average - 17.329 ms'
        const.time = []
